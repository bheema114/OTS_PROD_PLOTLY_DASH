...
def generate_chart(stations):
    ...
    fig = make_subplots(rows=1,cols=3,subplot_titles=["Total Stock", "Cards Sold"], specs=[[{'type': 'pie'}, {'type': 'bar'},{'type': 'bar'}]])
    ...
    if stations != 'All':
        filtered_values = dfst['stock'][(dfst['stock']['Category'] == 'No_of_Cards_Sold') & (dfst['stock']['Station Id'] == stations) ]['Count']
    ...
    bar_Card_sold = go.Bar(x=station_labels, y=filtered_values, name="Cards Sold",marker_color='rgb(60, 133, 120)',showlegend=False)
    
    bar_product_id = go.Bar(x=product_master, y=filtered_values, name="Product ID",marker_color='rgb(100, 22, 300)' ,showlegend=False) 
    ...
    fig.update_traces(textposition='inside', textinfo='label+percent+value', hole=0.6, hoverinfo="label+percent+value", selector=dict(type='pie'))
